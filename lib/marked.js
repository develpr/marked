...
(function(root) {
  ...
  var inline = {
    ...
    underline: /^_([^\s][\s\S]*?[^\s_])_(?!_)|^_([^\s_][\s\S]*?[^\s])_(?!_)|^_([^\s_])_(?!_)/,
  ...
  InlineLexer.prototype.output = function(src) {
    ...
    while (src) {
      ...
      if (
        ...
        console.log("MATCH reflink or nolink");
        src = src.substring(cap[0].length);
      ...
      if ((cap = this.rules.underline.exec(src))) {
        console.log("MATCH underline");
        src = src.substring(cap[0].length);
        out += this.renderer.underline(
      ...
      // text
      if ((cap = this.rules.text.exec(src))) {
        console.log("MATCH text with capture '" + cap[0] + "'");
        src = src.substring(cap[0].length);
  ...
  Renderer.prototype.underline = function(text) {
    return '<em class="underline">' + text + "</em>";
  };
  ...
  TextRenderer.prototype.strong = TextRenderer.prototype.em = TextRenderer.prototype.underline = TextRenderer.prototype.codespan = TextRenderer.prototype.del = TextRenderer.prototype.text = function(
    text
  ) {
    return text;
  };
